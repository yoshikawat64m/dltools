...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        # loss for all priors
        conf_loss = self._softmax_loss(
            y_true[:, :, 4:-8],
            y_pred[:, :, 4:-8]
        )
        loc_loss = self._l1_smooth_loss(
            y_true[:, :, :4],
            y_pred[:, :, :4]
        )
        ...
        pos_loc_loss = tf.reduce_sum(
            loc_loss * y_true[:, :, -8],
            axis=1
        )
        pos_conf_loss = tf.reduce_sum(
            conf_loss * y_true[:, :, -8],
            axis=1
        )
        ...
        full_indices = (
            tf.reshape(batch_idx, [-1]) * tf.to_int32(num_boxes) +
